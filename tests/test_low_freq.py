...
def run_low_frequency_sniper():
    ...
    rows = df_with_atr.select(["timestamp", "open", "high", "low", "close", "atr15"]).to_dicts()

    for target_rr in [2.0, 2.5, 3.0]:
        for max_trades_day in [1, 2]:
            ...
            for i in range(len(rows)):
                ...
                if i - last_exit_idx < 30: continue
                if daily_pnl[d] >= 100.0: continue
                if daily_trades[d] >= max_trades_day: continue

                # Prime hours only (London 08:00 - 10:30 UTC & NY 13:00 - 15:30 UTC)
                if not ((8, 0) <= (t.hour, t.minute) <= (10, 30) or (13, 0) <= (t.hour, t.minute) <= (15, 30)):
                    continue

                # Strong impulse candle check (High conviction only)
                body = abs(c - o)
                rng = h - l
                if rng < 0.50 or (body / rng) < 0.65: continue

                sl_dist = round(max(2.0, atr * 1.0), 2)
                tp_dist = round(sl_dist * target_rr, 2)
                lots = round(max(0.01, min(5.0, 50.0 / (sl_dist * 100))), 2)

                if c > o: # Bullish impulse
                    in_pos = ("BUY", c, round(c - sl_dist, 2), round(c + tp_dist, 2), lots, i)
                    daily_trades[d] += 1
                elif c < o: # Bearish impulse
            ...
            print(f"RR 1:{target_rr} | Max {max_trades_day} trade/d -> Trades: {n_tr} | Win: {wr:.1f}% | PF: {pf:.2f} | Net PnL: ${total_net:+,.2f} | Target Days: {target_days}")

if __name__ == "__main__":
    run_low_frequency_sniper()
