...
- Engine A (Trend Pullback) during Trending Regimes.
- Engine B (Liquidity Sweep) during Ranging Regimes.
- Daily Ratchet Risk Governor across the entire 10.5-month dataset (300,440 M1 bars).
"""
...
def main():
    ...
    res = engine.run_bars(df, dual_strat)
    ...
    for t in trades:
        ...
        if t.tag == "Trend_Engine":
            trend_trades += 1
        elif t.tag == "Range_Engine":
            range_trades += 1

    print("\n" + "=" * 85)
    print("XAUUSD DUAL-ENGINE: 10.5-MONTH DAY-BY-DAY INSTITUTIONAL AUDIT")
    print(f"Engine Allocation -> Trend Pullback: {trend_trades} trades | Range Sweep: {range_trades} trades")
    print("=" * 85)
