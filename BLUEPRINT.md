...
```
LLMTrading/
├── BLUEPRINT.md                 <- Master Architecture & Rules (File ini)
...
│   ├── processed/               <- Format Parquet terindeks
│   └── scripts/                 <- Exporter dari MT5 Wine
│       └── mt5_wine_exporter.py
├── engine/                      <- Custom Backtest & Simulation Engine
│   ├── core/                    <- Event-driven engine & order books
│   ├── metrics/                 <- Sharpe, Sortino, Drawdown, Profit Factor
│   ├── monte_carlo/             <- Monte Carlo permutation simulator
│   └── execution/               <- Slippage & spread dynamic models
├── agents/                      <- Multi-Agent Framework
│   ├── market_regime/           <- Regime & Intermarket detector
│   ├── researcher/              <- Strategy generator & code compiler
│   ├── validator/               <- Auditor & stress tester
│   ├── risk_manager/            <- Gatekeeper & position sizing
│   └── lifelong_learner/        <- Post-mortem & memory updater
├── strategies/                  <- Library Strategi Teruji
│   ├── incubator/               <- Strategi baru dalam pengujian
│   ├── active/                  <- Strategi yang lolos verifikasi institusi
│   └── retired/                 <- Strategi usang / performa turun
├── knowledge/                   <- Memory & Heuristics untuk Long-Life Learning
│   ├── playbooks/               <- SOP kondisi pasar (Chop, Trend, High Vol)
│   ├── learnings/               <- Catatan post-mortem kegagalan strategi
│   └── research_papers/         <- Rangkuman riset kuantitatif
...
- [ ] **Fase 1**: MT5 Wine Data Exporter (XAUUSD, XAGUSD, DXY - Bar & Tick data ke Parquet).
- [ ] **Fase 2**: Core Custom Backtest Engine (Vectorized + Event-Driven Tick Level).
- [ ] **Fase 3**: Monte Carlo Stress-Testing & Validation Module.
- [ ] **Fase 4**: Multi-Agent Implementation (Regime, Researcher, Validator, Risk).
- [ ] **Fase 5**: Scalping Strategy Incubator (XAUUSD Order Flow / Liquidity Sweep).
- [ ] **Fase 6**: Post-Mortem Feedback Loop & Knowledge Base Integration.
- [ ] **Fase 7**: Paper Trading & Live Bridge Execution.
