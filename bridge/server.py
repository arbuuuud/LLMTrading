...
class MultiTimeframeBarAggregator:
    ...
    def rebuild_m2_history(self):
        """
        Reconstructs M2 bars from current history_m1 bars after historical catch-up sync.
        """
        m2_dict = {}
        for m1 in self.history_m1:
            dt = m1["timestamp"]
            m2_min = (dt.minute // 2) * 2
            bucket = dt.replace(minute=m2_min, second=0, microsecond=0)
            bucket_sec = int(bucket.timestamp())
            if bucket_sec not in m2_dict:
                m2_dict[bucket_sec] = {
                    "symbol": m1["symbol"],
                    "timestamp": bucket,
                    "open": m1["open"],
                ...
                m2_dict[bucket_sec]["high"] = max(m2_dict[bucket_sec]["high"], m1["high"])
                m2_dict[bucket_sec]["low"] = min(m2_dict[bucket_sec]["low"], m1["low"])
                m2_dict[bucket_sec]["close"] = m1["close"]
                m2_dict[bucket_sec]["tick_volume"] += m1["tick_volume"]
    ...
    def process_tick(
        ...
        if completed_m1 is not None:
            ...
            # Aggregate into M2
            m2_min = (m1_dt.minute // 2) * 2
            m2_bucket_time = m1_dt.replace(minute=m2_min, second=0, microsecond=0)
            if self.current_m2_bucket is not None and m2_bucket_time != self.current_m2_bucket:
                if self.current_m2_bar is not None:
                    completed_m2 = dict(self.current_m2_bar)
                    if self.history_m2 and self.history_m2[-1]["timestamp"] == completed_m2["timestamp"]:
                        self.history_m2[-1] = dict(completed_m2)
                    else:
                        self.history_m2.append(dict(completed_m2))
                    if len(self.history_m2) > self.max_history_m2:
                        self.history_m2.pop(0)
                self.current_m2_bar = None

            self.current_m2_bucket = m2_bucket_time
            if self.current_m2_bar is None:
                self.current_m2_bar = {
                    "symbol": symbol,
                    "timestamp": m2_bucket_time,
                ...
                self.current_m2_bar["high"] = max(self.current_m2_bar["high"], completed_m1["high"])
                self.current_m2_bar["low"] = min(self.current_m2_bar["low"], completed_m1["low"])
